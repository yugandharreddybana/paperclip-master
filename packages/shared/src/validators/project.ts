...
const githubRepositoryLinkSchema = z
  .object({
    fullName: z
      .string()
      .min(1)
      ...
      .refine((value) => value.includes("/") && value.split("/").length === 2, "Expected owner/repo"),
    htmlUrl: z.string().url().optional(),
    defaultBranch: z.string().max(256).optional().nullable(),
  })
  .strict();

export const githubRepositoryLinksFieldSchema = z
  .array(githubRepositoryLinkSchema)
  .max(50)
  .superRefine((entries, ctx) => {
    const seen = new Set<string>();
    for (let i = 0; i < entries.length; i++) {
      const key = entries[i]!.fullName.toLowerCase();
      if (seen.has(key)) {
        ctx.addIssue({
...
export type GithubRepositoryLinkValidated = z.infer<typeof githubRepositoryLinkSchema>;
...
const projectFields = {
  ...
  githubRepositoryLinks: githubRepositoryLinksFieldSchema.optional(),
