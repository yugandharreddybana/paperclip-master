import type { GithubRepositoryLink, Goal } from "@paperclipai/shared";
...
export function buildOnboardingProjectPayload(
  goalId: string | null,
  githubRepositoryLinks?: GithubRepositoryLink[] | null,
) {
  const normalizedLinks = (githubRepositoryLinks ?? [])
    .map((link) => ({
      fullName: link.fullName.trim(),
      ...(link.htmlUrl ? { htmlUrl: link.htmlUrl } : {}),
      ...(link.defaultBranch ? { defaultBranch: link.defaultBranch } : {}),
    }))
    .filter((link) => link.fullName.length > 0);
  ...
  return {
    ...
    ...(normalizedLinks.length > 0
      ? { githubRepositoryLinks: normalizedLinks }
      : {}),
  };
}
