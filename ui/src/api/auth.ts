...
export const authApi = {
  ...
  /**
   * Link GitHub to the signed-in board user (Better Auth session).
   * Requires `deploymentMode: authenticated` and `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET`.
   * Redirects the browser to GitHub; callback returns to `callbackURL`.
   */
  linkGithubAccount: async (input?: { callbackURL?: string }) => {
    const callbackURL =
      input?.callbackURL ??
    ...
    const url = typeof (payload as { url?: unknown })?.url === "string" ? (payload as { url: string }).url : null;
    if (!url) {
