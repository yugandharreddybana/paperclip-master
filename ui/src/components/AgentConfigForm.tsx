...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const ollamaModelsProbe = useMemo(() => {
    if (adapterType !== "ollama_local") return { baseUrl: "", apiKey: "" };
    if (isCreate) {
      return {
        baseUrl: String(val?.adapterSchemaValues?.baseUrl ?? "").trim(),
        apiKey: String(val?.adapterSchemaValues?.apiKey ?? "").trim(),
      };
    }
    const ac = { ...config, ...overlay.adapterConfig } as Record<string, unknown>;
    return {
      baseUrl: String(ac.baseUrl ?? "").trim(),
      apiKey: String(ac.apiKey ?? "").trim(),
    };
  }, [adapterType, isCreate, val?.adapterSchemaValues, config, overlay.adapterConfig]);

  // Fetch adapter models for the effective adapter type
  const modelQueryKey = selectedCompanyId
    ? [
        ...queryKeys.agents.adapterModels(selectedCompanyId, adapterType, currentDefaultEnvironmentId || null),
        adapterType === "ollama_local" ? `${ollamaModelsProbe.baseUrl}\0${ollamaModelsProbe.apiKey}` : "",
      ]
  ...
  const {
    ...
    queryFn: () =>
      agentsApi.adapterModels(selectedCompanyId!, adapterType, {
        environmentId: currentDefaultEnvironmentId || null,
        ...(adapterType === "ollama_local" && ollamaModelsProbe.baseUrl
          ? { ollamaBaseUrl: ollamaModelsProbe.baseUrl }
          : {}),
        ...(adapterType === "ollama_local" && ollamaModelsProbe.apiKey
          ? { ollamaApiKey: ollamaModelsProbe.apiKey }
          : {}),
      }),
    enabled: Boolean(selectedCompanyId),
    staleTime: 0,
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
                ...
                (refreshModelsError || fetchedModelsError) && (
                  <p className="text-xs text-destructive">
                    {refreshModelsError
                      ?? (fetchedModelsError instanceof Error
                        ? fetchedModelsError.message
                        : "Failed to load adapter models from Ollama.")}
                  </p>
                )
              )}
              ...
              <uiAdapter.ConfigFields
                ...
                {...(adapterType === "ollama_local"
                  ? {
                      adapterModelRefresh: handleRefreshModels,
                      adapterModelRefreshing: refreshingModels,
                    }
                  : {})}
              />
