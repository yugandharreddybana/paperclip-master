        ...
        .map((field) => {
          switch (field.type) {
            ...
            case "combobox": {
              ...
              if (field.meta?.useAdapterModels === true && models.length > 0) {
                comboboxOptions = models.map((m) => ({
                  label: m.label,
                  value: m.id,
                }));
              }
              ...
              const showModelRefresh =
                field.meta?.useAdapterModels === true && typeof adapterModelRefresh === "function";
              return (
                <Field key={field.key} label={field.label} hint={field.hint}>
                  <div className="flex gap-2 items-start">
                    ...
                    {showModelRefresh ? (
                      <button
                        ...
                        onClick={() => void adapterModelRefresh()}
                        disabled={adapterModelRefreshing}
                      >
                        {adapterModelRefreshing ? "Refreshing…" : "Refresh"}
                      </button>
                    ) : null}
                  </div>
                  {field.meta?.useAdapterModels === true && models.length === 0 ? (
                    <p className="text-xs text-muted-foreground mt-1.5">
                      No models loaded yet. Start Ollama, check the server URL above, then Refresh — or type a model tag manually.
                    </p>
                  ) : null}
                </Field>
              );
