...
import { listOllamaModels } from "@paperclipai/adapter-ollama-local/server";
import {
...
export function agentRoutes(
  ...
  router.get("/companies/:companyId/adapters/:type/models", async (req, res) => {
    ...
    // Ollama listing only probes the HTTP API (baseUrl / apiKey). It must not be
    // gated on `environmentId`: the UI always sends the agent's default env for cache
    // keying, but a stale or missing env would 404 and yield an empty model list.
    if (type === "ollama_local") {
      const baseUrlQ = typeof req.query.baseUrl === "string" ? req.query.baseUrl.trim() : "";
      const apiKeyQ = typeof req.query.apiKey === "string" ? req.query.apiKey.trim() : "";
      const cfg: Record<string, unknown> = {};
      if (baseUrlQ) cfg.baseUrl = baseUrlQ;
      if (apiKeyQ) cfg.apiKey = apiKeyQ;
